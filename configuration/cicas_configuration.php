...
function configuration_cicas_configuration()
{
	...
	//ajout erasme
	$cicasstatutcrea = $GLOBALS['ciconfig']['cicasstatutcrea'];
    ...
    // cicasuid
    $ci_tableau_uid = array("LaclasseEmail" => "LaclasseEmail", "LOGIN" => "LOGIN");
    if (!$cicasuid)
    	$cicasuid = "email";
   ...
   // cicasstatutcrea
    $tab_cicasstatutcrea = array("0minirezo" => "administrateur" , "1comite" => "redacteur" , "6forum" => "forum"  );
	
	$res .= "\n<tr><td class='verdana2'>"
	. "<label for='cicasuid'>Statut à la création</label><br />"
    . "\n<select name='cicasstatutcrea' class='formo' ".($ciedit ? "" : "disabled='disabled'"). ">\n"
    . "<option value='$cicasstatutcrea' selected='selected'>".$tab_cicasstatutcrea[$cicasstatutcrea]."</option>\n";
    
	foreach ($tab_cicasstatutcrea as $cle => $valeur) {
	if ($cle <> $cicasstatutcrea)
		$res .= "<option value='$cle'>".$valeur."</option>\n";
	}
	
	$res .= "<option value='non'>Pas de création</option>\n";
